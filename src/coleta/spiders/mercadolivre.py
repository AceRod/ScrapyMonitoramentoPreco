...
class MercadolivreSpider(scrapy.Spider):
    ...
    start_urls = ["https://lista.mercadolivre.com.br/tenis-masculinos#D[A:tenis%20masculinos]"]
    page_count = 1
    max_pages = 10
    ...
    def parse(self, response):
        ...
        for product in products:
            ...
            yield {
                    ...
                    'old_price_cent': cents[0] if len(cents) > 0 else None,
                    'new_price': prices[1] if len(prices) > 1 else None,
                    'new_price_cent': cents[1] if len(cents) > 1 else None,
